...
export default class AddProduct extends Component {
  ...
  fetchStartStop = async () => {
    try {
      const params = null;
      const response = await makeRequest(
        BASE_URL + 'api/Vendors/startTakingOrderTimeList',
        params,
        true,
        false,
      );
  ...
  handleAddProduct = async () => {
    ...
    try {
      ...
      const startTakingHour = isAllTimeAvailable ? null : selectedTakingTime.Id;
  ...
  handleSelectStartTakingOrderClose = () => {
    const {selectedTakingTime} = this.state;
    this.setState({selectedTakingTime});
  };

  renderStartTakingOrderCategoryPicker = (disabled, selected, showModal) => {
    const {selectedTakingTime} = this.state;
    const {Name} = selectedTakingTime;
      ...
      <SafeAreaView style={[basicStyles.container, {backgroundColor: '#fff'}]}>
        ...
        <View style={[styles.container]}>
          <View style={styles.available24}>
            <Text style={basicStyles.headingLarge}>Product Available 24*7</Text>
            {/* <Switch
